...
fbRouter.post("/feedback",async(req,res) => { //route handler for POST requests to the "/feedback" endpoint
  try {
    const sql = "insert into feedback (user_id , email, feedback_text, satisfaction_rating ) values ($1,$2,$3,$4) returning id_feedback"
    const result = await query(sql,[req.body.user_id,req.body.email,req.body.feedback_text, req.body.satisfaction_rating])
